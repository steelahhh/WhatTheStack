...
class WhatTheStackInitializer : Initializer<WhatTheStackInitializer.InitializedToken> {
   ...
   *
   * 1. Start [WhatTheStackService] as a bound service to allow communication between the app's
   * process and the service's process
   * 2. Replace the app's default [Thread.UncaughtExceptionHandler] with
   * [WhatTheStackExceptionHandler] when the service is connected.
   *
   * This method does not need to return anything, but it is required to return a sensible value
   * here so we return a dummy object [InitializedToken] instead.
   */
      ...
      object : ServiceConnection {
        override fun onServiceConnected(name: ComponentName?, service: IBinder) {
          ...
          val customExceptionHandler =
            WhatTheStackExceptionHandler(messenger, defaultExceptionHandler)
